...
// first pass: open the MPP, recover task skeleton/name/progress data, and return quickly.
const MODULES = [
...
self.onmessage = async (event) => {
  ...
  try {
    progress(24, 'Fast worker ready', 'Loaded the fast browser-only MPP importer with VarMeta names and % Complete recovery.');
    ...
    cleaned.warnings.unshift('Fast MPP import mode: loaded the first usable schedule quickly with native VarMeta task-name and % Complete recovery. Deep native resources, assignments, and date scans are skipped on live upload for speed.');
